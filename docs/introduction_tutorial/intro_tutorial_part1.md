...
While optional, if you want to interact with the `FilePattern` object a bit more you can iterate through it using `.items()`. To keep the output concise, we use an if statement to stop the iteration after a few filepaths.
